...
@file:cifar10_resnext.py
@author: losstie
@create_time: 2019/5/27 21:23
@description:
"""
...
# Running the model
###############################################################################
class Cifar10Model(ResNeXt_model.Model):
    """Model class with appropriate defaults for CIFAR-10 data. """

    def __init__(self, resnext_size, cardinality,data_format=None, num_classes=NUM_CLASSES,
                 resnext_version=ResNeXt_model.DEFAULT_VERSION,
                 dtype=ResNeXt_model.DEFAULT_DTYPE):
        """These are the parameters that work for `CIFAR-10` data
        Args:
            resnext_size: The number of convolution layers needed in the model.
            ...
            resnext_version:Integer representing which version of the ResNeXt network to use.
        ...
        """
        if resnext_size % 6 != 2:
            raise ValueError('resnet_size must be 6n+2:', resnext_size)

        num_blocks = (resnext_size - 2) // 6
        ...
        super(Cifar10Model, self).__init__(
            ...
            block_stride=[1, 2, 2],
            resnext_version=resnext_version,
...
def cifar10_model_fn(features, labels, mode, params):
    ...
    return resnext_model_fn(
        features=features,
        labels=labels,
        mode=mode,
        model_class=Cifar10Model,
        cardinality=params['cardinality'],
        resnext_size=params['resnext_size'],
        weight_decay=weight_decay,
        learning_rate_fn=learning_rate_fn,
        momentum=0.9,
        data_format=params['data_format'],
        resnext_version=params['resnext_version'],
...
def resnext_model_fn(features, labels, mode, model_class, resnext_size, cardinality,
                    weight_decay, learning_rate_fn, momentum,
                    data_format,resnext_version, loss_scale,
                    loss_filter_fn=None, dtype=ResNeXt_model.DEFAULT_DTYPE,
                    fine_tune=False, label_smoothing=0.0):
    model = model_class(resnext_size, cardinality, data_format=data_format, resnext_version=resnext_version, dtype=dtype)
...
def resnext_main(flags_obj, model_function, input_function, dataset_name, shape=None):
    """"""
    ...
    classifier = tf.estimator.Estimator(
        ...
        warm_start_from=warm_start_settings, params={
            'resnext_size': int(flags_obj.resnext_size),
            'cardinality': int(flags_obj.cardinality),
            'data_format': flags_obj.data_format,
            'batch_size': flags_obj.batch_size,
            'resnext_version': flags_obj.resnext_version,
            'loss_scale': 1,
            'dtype': tf.float32,
...
def main(unused_argv):
    resnext_main(flags.FLAGS, cifar10_model_fn, input_fn, DATASET_NAME,
                shape=[HEIGHT, WIDTH, NUM_CHANNELS])


def define_flags(resnext_size_choices=None):
    ...
    flags.DEFINE_enum(name="resnext_version", short_name="rv",
    ...
    choice_kwargs = dict(
        name='resnext_size', short_name='rs', default='56',
        help='The size of the ResNeXt model to use.')

    if resnext_size_choices is None:
        flags.DEFINE_string(**choice_kwargs)
    else:
        flags.DEFINE_enum(enum_values=resnext_size_choices, **choice_kwargs)
